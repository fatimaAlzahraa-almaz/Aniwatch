...
const AnimeDetailesPage = () => {
  const { favoritesList, toggleFavorites } = useFavoritesStore();
  const { id } = useParams();
  const animeId: number = Number(id);
  const { data, isLoading, error, refetch } = useAnimeInfo({
    id: animeId,
  });
  const navigate = useNavigate();
  const handleWatchClick = () => {
    navigate("/info/" + data?.Media?.id);
  };
  const handleHomeClick = () => {
    navigate("/");
  };
  const isFav = favoritesList.some((item) => item.id === animeId);
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [id]);
    ...
    <div className="w-full mt-16 flex flex-col gap-5 p-2">
      <div className="flex   sm:flex-row flex-col gap-5 items-center sm:items-start ">
        <div className="sm:w-[16rem] border relative border-amber-200/30 overflow-hidden rounded-xl sm:h-[20rem] min-w-0 w-[12rem] h-[16rem]">
          <img
            className="sm:hover:scale-105 absolute inset-0 h-full w-full object-cover object-center rounded-xl duration-150"
            src={data?.Media?.coverImage?.large ?? undefined}
          />
        </div>
        ...
        <div className="flex flex-col gap-y-5 sm:gap-y-6  w-full">
          ...
          <p className="font-light text-sm sm:text-lg text-gray-300">
            {data?.Media?.description ? parse(data?.Media?.description) : ""}
          </p>
          <div className="flex gap-2 flex-wrap">
            {data?.Media?.genres?.map((el: string, i: number) => {
              return (
                <p
                  key={i}
                  className=" border border-yellow-200/40  p-1 rounded-3xl text-sm font-light px-2"
                >
                  {el}
                </p>
              );
          ...
          <div className="flex gap-1 sm:gap-2 text-black text-sm sm:text-lg  sm:justify-normal justify-center flex-wrap">
            <button
              ...
              className="bg-yellow-200  px-2 sm:px-4 rounded-3xl flex  items-center justify-center p-2 hover:bg-yellow-300 cursor-pointer pr-3 min-w-0 truncate"
            >
              <Play className="fill-black w-5   " />
              Watch now
            </button>
            {isFav ? (
              <button
                onClick={() => {
                  if (data?.Media) toggleFavorites(data?.Media);
                }}
                className="bg-white px-2  sm:px-4 rounded-3xl flex items-center justify-center p-2 hover:bg-gray-100 cursor-pointer pr-3 min-w-0 truncate"
              >
                <Added className=" w-5" />
                Added
              </button>
      ...
      <div>
        {data?.Media?.trailer?.id &&
          data?.Media?.trailer?.site == "youtube" && (
            <iframe
              title="Anime trailer"
              className="w-full sm:h-120 h-80"
...
export default AnimeDetailesPage;
