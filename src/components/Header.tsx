...
const Header = () => {
  ...
  const hundleHomeSubmit=()=>{
    navigate('/');
  }
  const hundleFormSubmit=(e:React.SubmitEvent<HTMLFormElement>)=>{
   e.preventDefault();
   if(que.trim()){
  ...
  const hundleHideFormSubmit=(e:React.SubmitEvent<HTMLFormElement>)=>{
    hundleFormSubmit(e);
  ...
  return (
    <header  className='fixed flex right-0 top-0 left-0 h-16 items-center justify-between  sm:text-xl font-semibold   bg-gray-950/80 z-100 sm:px-2 px-1 '>
      <div className='flex items-center gap-1 sm:gap-2 w-full justify-between  sm:justify-start'>
        ...
        <div onClick={hundleHomeSubmit} className='flex hover:cursor-pointer justify-center items-center '>
        ...
        <form onSubmit={hundleFormSubmit} className=' hidden sm:flex   bg-gray-100  rounded-3xl items-center text-lg  text-black'>
      ...
      }
      {
        ...
        className='sm:hidden absolute top-0 w-full bg-black px-1 pb-2 left-0 right-0 '>
          ...
          <form onSubmit={hundleHideFormSubmit} className='flex   bg-gray-100     rounded-3xl items-center  text-black'>
        <input onChange={(e)=>setQue(e.target.value)} value={que}
