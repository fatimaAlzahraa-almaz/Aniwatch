...
const Colcard = ({obj}:{obj:animeListType}) => {
  ...
  return (
    ...
    >
      <div   className=' h-[7rem]   w-[5rem]   rounded-xl cursor-pointer group   relative '>
        <div onClick={hundleClick} className='w-full h-full relative overflow-hidden rounded-xl '>
           <img 
            onMouseEnter={()=>setHover(true)}
            onMouseLeave={()=>setHover(false)}
           className={`absolute inset-0 w-full h-full object-cover objetc-center rounded-xl  ${hover ? 'scale-110':'scale-100' } duration-200`} src={obj?.coverImage?.large ?? undefined}/>
      ...
      <p onClick={hundleClick} className='sm:text-base  text-sm  line-clamp-2  pb-1 hover:text-yellow-200 cursor-pointer '>{obj?.title?.romaji ?? 'N/A'}</p>
      <div className='flex  text-sm  gap-[2.5px]  items-center text-gray-800 font-base   min-w-0'>
        ...
        <div className='flex items-center  h-5  bg-emerald-200/90 rounded-bl-sm rounded-tl-sm gap-0.5 px-1 line-clamp-1'>
          ...
          <p className='text-xs   '>{obj?.duration ? obj?.duration+'m' :'N/A' }</p>
        </div>
        ...
        <p className='text-gray-200 font-extralight line-clamp-1 min-w-0 truncate'>{obj?.format ?? 'N/A'}</p>
      </div>
