...
const TrendingAnimeCard = ({ obj, i }: { obj: animeType; i: number }) => {
  ...
  return (
    ...
    >
      <div className="w-full sm:h-[20rem] h-[14rem] overflow-hidden relative rounded-t-xl group">
        <div
          onClick={handleClick}
          className="absolute z-2 rounded-t-xl cursor-pointer opacity-5 bg-linear-to-t from-black inset-0 sm:group-hover:hidden "
        ></div>
        <img
          className=" absolute inset-0 w-full h-full  object-cover object-center rounded-t-xl  group-hover:scale-105   duration-200 delay-100 sm:group-hover:blur-md "
        ...
        <div className="top-0 w-full h-full absolute  translate-y-[100%] text-gray-300   text-sm p-3 bg-gray-800/40 group-hover:translate-y-0 group-hover:transition group-hover:duration-400  cursor-pointer hidden sm:block z-3 flex flex-col">
          <div onClick={handleClick} className="w-full h-full  ">
            <p className="font-semibold pb-2 text-white line-clamp-1 truncate">
              {obj?.title?.romaji ?? ""}
            </p>
            <p className="font-extralight pb-1 line-clamp-8">
            ...
            <div className="flex flex-wrap">
              <p>Aired:</p>
              <p className="font-extralight text-white">
                {obj?.startDate?.year ?? "N/A"}
              </p>
            </div>
            <div className="flex">
              <p>Genres:</p>
              <p className="font-extralight text-white">
                {obj?.genres ? obj?.genres[0] : "N/A"}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between pt-3 font-medium  absolute right-1 left-1 bottom-2">
            <button
              onClick={handleClick}
              className="bg-yellow-200 flex text-black items-center justify-center py-1 w-[11rem] rounded-2xl hover:bg-yellow-300 cursor-pointer "
            >
              {" "}
            ...
            <button
              ...
              className="bg-gray-100 rounded-full    hover:bg-gray-200 w-8 h-8 flex items-center justify-center cursor-pointer "
            >
        ...
        <div className="absolute top-2 left-2 flex sm:group-hover:opacity-0 ">
          <p className="bg-gray-600/50 rounded-xl   sm:text-base text-sm py-1  px-3 font-semibold">
            {i < 4 ? `#0` + i : i > 9 ? i : `0` + i}
          </p>
        </div>
      </div>
      ...
      <div className="w-full p-1  ">
        <p
          onClick={handleClick}
          className="max-w-[20ch] truncate  cursor-pointer  "
        >
          {obj?.title?.romaji ?? ""}
        </p>
