...
const AnimeCard = ({ obj }: { obj: animeType }) => {
    ...
    <div className="bg-gray-800/70  aspect-2/3  text-sm flex flex-col relative rounded-xl    ">
      <div className="w-full  h-[85%]  cursor-pointer group relative">
        ...
        >
          <div
            onClick={hundleClick}
            className="absolute z-2 w-full cursor-pointer opacity-10 h-full bg-linear-to-t from-black rounded-t-xl  "
          ></div>
          <img
            className="  group-hover:scale-105 transition duration-150  sm:group-hover:blur-sm   absolute inset-0 w-full h-full object-cover object-center rounded-t-xl "
        ...
        <div className="absolute bottom-0  text-xs flex w-full  font-medium text-center p-1 tracking-tight leading-tight min-w-0">
          ...
          <p className="bg-gray-600/70 rounded-br-sm rounded-tr-sm text-white p-0.5 max-w-[18ch] truncate min-w-0 ">
      ...
      <div className="w-full sm:p-2 p-1.5 flex-col ">
        <p
          onClick={hundleClick}
          className="sm:text-base text-sm  sm:max-w-none max-w-[19ch] truncate cursor-pointer hover:text-yellow-300 pb-1"
        >
          {obj?.title?.romaji ?? ""}
        </p>
