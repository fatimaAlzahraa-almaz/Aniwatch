...
import type { rowPropsType } from "../../types/types";
import LoadingAnimeCard from "../skeleton/LoadingAnimeCard";
const AnimeRow = ({
  obj,
  loading,
  genre,
  ...
  const handleClick = () => {
    navigate(
      `/explore?genre=${genre ?? ""}&sort=${sort ?? ""}&status=${status ?? ""}&perPage=30&page=1`,
      { state: { title: title } },
    );
  };
  return (
    <div className="flex flex-col  w-full p-2">
      <div className="flex mb-4 justify-between items-center flex-wrap w-full">
        <p className=" text-yellow-200 sm:text-2xl  text-lg font-semibold min-w-0 truncate ">
          {title}
        </p>
